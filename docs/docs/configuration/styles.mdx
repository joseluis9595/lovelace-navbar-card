...
:::tip
You can check out some examples [**here**](/docs/examples) for inspiration.
:::
...
- `.badge`: Small indicator or badge that appears over the icon (if configured).
  ...
  - `.badge.with-counter`: Applies when the badge displays text or a number.
  - `.badge.with-icon`: Applies when the badge displays an icon.
  - `.badge-icon`: Refers to the `ha-icon` displayed inside an icon badge.
...
| `--navbar-badge-size`                 | 16px                                 | Size of badges that contain a count, text, or icon                |
| `--navbar-badge-icon-size`            | 12px                                 | Size of the icon displayed inside an icon badge                   |
...
## Badge sizing example

You can increase an icon badge while keeping the same icon-to-badge proportions:

```yaml
styles: |
  .navbar {
    --navbar-badge-size: 18px;
    --navbar-badge-icon-size: 13.5px;
  }
```
